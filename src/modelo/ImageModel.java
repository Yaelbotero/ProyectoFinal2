...
import static modelo.ColorBlindType.DEUTERANOPIA;
import static modelo.ColorBlindType.PROTANOPIA;
import static modelo.ColorBlindType.TRITANOPIA;

/**
 * MODELO — ImageModel
 * 
 * Contiene toda la lógica de negocio para simular los tipos de daltonismo.
 * Utiliza matrices de transformación LMS (Long-Medium-Short) basadas en
 * investigación científica de Brettel, Viénot & Mollon (1997) y
 * Machado, Oliveira & Fernandes (2009).
 * 
 * El pipeline de transformación es:
 *   1. RGB lineal → espacio LMS (conos del ojo humano)
 *   2. Aplicar matriz de simulación según tipo de daltonismo
 *   3. LMS modificado → RGB lineal
 *   4. Corrección gamma → RGB final
 */
public class ImageModel {
    ...
    // ─────────────────────────────────────────────
    // Matrices de conversión RGB ↔ LMS
    // Basadas en el espacio de color de Hunt-Pointer-Estévez (HPE)
    // adaptado al iluminante D65
    // ─────────────────────────────────────────────
    ...
    /** Protanopía: elimina la función del cono L (rojo) */
    ...
    /** Deuteranopía: elimina la función del cono M (verde) */
    ...
    /** Tritanopía: elimina la función del cono S (azul) */
    ...
    /**
     * Aplica la simulación de daltonismo a la imagen original.
     * 
     * @param type Tipo de daltonismo a simular
     * @return Imagen procesada con los colores transformados
     */
    ...
    // ─────────────────────────────────────────────
    // Pipeline de transformación de color
    // ─────────────────────────────────────────────

    /**
     * Transforma un píxel aplicando el pipeline completo:
     * RGB → linearización → LMS → simulación → RGB → gamma
     */
    private Color transformColor(Color color, double[][] simMatrix) {
        // 1. Obtener componentes RGB [0.0, 1.0]
        double r = color.getRed();
        double g = color.getGreen();
        double b = color.getBlue();

        // 2. Linearizar (eliminar corrección gamma sRGB)
        r = linearize(r);
        g = linearize(g);
        b = linearize(b);
    ...
    /**
     * Multiplica un vector de 3 elementos por una matriz 3x3.
     */
    ...
    /**
     * Lineariza un valor sRGB (elimina la corrección gamma).
    ...
    /**
     * Aplica corrección gamma sRGB a un valor lineal.
     */
    ...
    /**
     * Retorna la matriz de simulación correspondiente al tipo de daltonismo.
     */
    private double[][] getSimulationMatrix(ColorBlindType type) {
    switch (type) {
        case PROTANOPIA:
            return PROTANOPIA_LMS;
        case DEUTERANOPIA:
            return DEUTERANOPIA_LMS;
        case TRITANOPIA:
            return TRITANOPIA_LMS;
        default:
